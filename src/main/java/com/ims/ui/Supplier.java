...
public class Supplier {
    ...
    private void tableLoad() {
        try {
            loadTable = conn.prepareStatement("SELECT * FROM supplier" );
            ResultSet results = loadTable.executeQuery();
            tableSupplier.setModel(DbUtils.resultSetToTableModel(results));
    ...
    private void clickTableRow() {
        ...
        int column = tableSupplier.getColumnCount();
        Object[] val = new Object[column];
        for(int i = 0; i < column; i++) {
            val[i]=tableSupplier.getValueAt(row, i);
        }
    ...
    private void clickAddButton() throws SQLException {
        if (checkAllTextFieldsAreFilled())
            ...
            new SupplierDAO().addSupplierDAO(transferTextFieldToSupplierDTOWithoutSearchID());
            clearTextFields();
            tableLoad();
        }
    }
    ...
    private void clickSearchButton() throws SQLException {
        ...
        try {
            var list = new SupplierDAO().searchSupplierID(Integer.parseInt(textFieldSearchID.getText()));
    ...
    private void clickUpdateButton() throws SQLException {
        try {
            if (checkAllTextFieldsAreFilled())
                ...
                new SupplierDAO().updateSupplier(transferTextFieldToSupplierDTO());
                clearTextFields();
                tableLoad();
            }
        }catch (Exception e) {
    ...
    private void clickDeleteButton() throws SQLException {
        try {
            new SupplierDAO().deleteSupplier(Integer.parseInt(textFieldSearchID.getText()));
            clearTextFields();
            tableLoad();
        }catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    private SupplierDTO transferTextFieldToSupplierDTO() {
        SupplierDTO supplierDTO = new SupplierDTO();
        supplierDTO.setSupplierId(Integer.parseInt(textFieldSearchID.getText()));
        supplierDTO.setSupplierName(textFieldName.getText());
        supplierDTO.setCountry(textFieldCountry.getText());
        supplierDTO.setState(textFieldState.getText());
        supplierDTO.setAddress(textFieldAddress.getText());
        supplierDTO.setEmail(textFieldEmail.getText());
        return supplierDTO;
    }

    private SupplierDTO transferTextFieldToSupplierDTOWithoutSearchID() {
        SupplierDTO supplierDTO = new SupplierDTO();
        supplierDTO.setSupplierName(textFieldName.getText());
        supplierDTO.setCountry(textFieldCountry.getText());
        supplierDTO.setState(textFieldState.getText());
